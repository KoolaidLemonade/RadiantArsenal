...
{
    public class AquasteelSpellblade : RadianceItem
    {
        bool isActive;
        float activeTimer;
        public override void SetStaticDefaults()
        {
            Tooltip.SetDefault("Right click to summon spellblade" + "\nSprays water at nearby enemies" + "\nLasts 8 seconds");
        }
        public override void SafeSetDefaults()
        {
            ...
            item.autoReuse = true;
            item.shoot = mod.ProjectileType("AquasteelSpellbladeProj");
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            else
            {
                ...
                Projectile.NewProjectile(player.Center, perturbedSpeed * 3, mod.ProjectileType("AquasteelSpellbladeProj"), damage, knockBack, player.whoAmI);
        ...
        {
            if (player.altFunctionUse == 2 && player.itemAnimation > 0 && player.HeldItem.type == mod.ItemType("AquasteelSpellblade"))
            {
                isActive = true;
            }
            ...
            if (isActive)
            {
                ...
                for (int i = 0; i < 200; i++)
                {
                    ...
                    if (npc.Distance(player.Center) < 450 && npc.active && !npc.dontTakeDamage && !npc.immortal && !npc.friendly && npc.lifeMax > 5)
                    {
                        ...
                        if (activeTimer % 20 == 0)
                        {
                            Projectile.NewProjectile(player.Center, (npc.Center - new Vector2(0, 60) - player.Center) / 25, ProjectileID.WaterStream, item.damage / 4, item.knockBack, player.whoAmI);
                        }
                    }
                }
            }
