...
{
    public class StarseekerSpellblade : RadianceItem
    {
        bool isActive;
        float activeTimer;
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            else
            {
                ...
                Projectile.NewProjectile(player.Center, perturbedSpeed * Main.rand.NextFloat(3, 8), mod.ProjectileType("StarseekerSpellbladeProj"), damage, knockBack, player.whoAmI);
        ...
        {
            if (player.altFunctionUse == 2 && player.itemAnimation > 0 && player.HeldItem.type == mod.ItemType("StarseekerSpellblade"))
            {
                isActive = true;
            }
                ...
                for (int i = 0; i < 200; i++)
                {
                    ...
                    if (npc.Distance(player.Center) < 450 && npc.active && !npc.dontTakeDamage && !npc.immortal && !npc.friendly && npc.lifeMax > 5)
                    {
                        ...
                        if (Main.rand.NextFloat() < 0.02f)
                        {
                            ...
                            Projectile.NewProjectile(spawnPos, (npc.Center - spawnPos) / 40, mod.ProjectileType("StarseekerSpellbladeProj2"), item.damage, item.knockBack, player.whoAmI);
                        }
                    }
                }
            }
